...
TcpServer::TcpServer(const std::string &ip, const uint16_t port, int threadnum):threadnum_(threadnum),mainevl_(new EventLoop)
,acceptor_(mainevl_,ip,port),threadpool_(threadnum_, "IO")
{
    //创建主事件
    ...
    for(int ii = 0; ii < threadnum_; ii++){
        subevls_.emplace_back((new EventLoop()));
...
void TcpServer::newconnection(std::unique_ptr<Socket> clientsock){
    ...
    spConnection conn(new Connection(subevls_[clientsock->getfd()%threadnum_], std::move(clientsock)));
