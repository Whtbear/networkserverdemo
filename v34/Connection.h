...
private:
    ...
    Timestamp lasttime_;    //创建Connection对象时赋值当前时间，接收到报文时更新当前时间戳

public:
    Connection(EventLoop *evl, std::unique_ptr<Socket> clientsock);
    ~Connection();
    ...
    void send(const char *data,size_t size);//任何线程都需要调用此函数
    void sendinloop(const char *data,size_t size);//需要交给IO线程来发送数据

    bool timeout(time_t now, int threshold);   //判断TCP连接是否超时(空闲太久)。
};